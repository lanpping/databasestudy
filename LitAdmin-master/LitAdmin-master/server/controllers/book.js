...
bookController.find = function (req, res) {
  ...
  connection.query(' select Count(*) as num from book ;SELECT * from book limit 0,10', function (error, results, fields) {
    if (error) throw error;
    console.log('--------------------------Select----------------------------');
    console.log(results[0])
    total = JSON.stringify(results[0]);//把results对象转为字符串，去掉RowDataPacket
    total = JSON.parse(total);//把results字符串转为json对象
    total = total[0].num
    data = JSON.stringify(results[1]);//把results对象转为字符串，去掉RowDataPacket
    rltBooks = JSON.parse(data);//把results字符串转为json对象
    // console.log('The solution is: ', rltBooks);
    res.json({
      total: total,
      limit: limit,
      books: rltBooks
