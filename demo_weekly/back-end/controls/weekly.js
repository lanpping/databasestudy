    ...
    $http.userVerify(req, res, () => {
      ...
      if(!startTime || !title || !buyer) $http.writeJson(res, {code: 2, message:'参数有误'})
        ...
        $http.connPool(sql, arrayParams, (err, result) => {
          ...
          return $http.writeJson(res, {code: 1, message: '添加订单成功'})
        })
      }
    })
  },
...
// 自己加的-----------------------------------------------------------------------------------------------------
      ...
      if(!id||!startTime || !title || !userId || !buyer) {
        ...
        $http.connPool(sql, arrayParams, (err, result) => {
          ...
          return $http.writeJson(res, {code: 1, data: result, message: '更新订单成功'})
        })
      }
    })
  },
  ...
  updateState (req, res) {
    ...
    $http.userVerify(req, res, () => {
      ...
      if(!id) {$http.writeJson(res, {code: 2, message:'参数有误'})}
        ...
        $http.connPool(sql, arrayParams, (err, result) => {
          if(err) return $http.writeJson(res, {code:-2, message:'失败',errMsg: err})
          if(result.affectedRows != 1) return $http.writeJson(res, {code: 2, data: result, message:'更新订单状态失败'})
          return $http.writeJson(res, {code: 1, message: '更新订单状态成功'})
        })
      }
    })
  },
    ...
    $http.userVerify(req, res, () => {
      ...
      if(!id) {$http.writeJson(res, {code: 2, message:'参数有误'})}
        ...
        $http.connPool(sql, arrayParams, (err, result) => {
          if(err) {return $http.writeJson(res, {code:-2, message:'失败',errMsg: err})}
          if(result.length != 1) {
            return $http.writeJson(res, {code: 2, message:'获取订单信息不存在'})
          } else {
            let resultData = formatData(result)[0]
            return $http.writeJson(res, {code: 1, data: resultData, message: '获取订单信息成功'})
          }
        })
      }
    })
    ...
    //  let searchEndTime = params.searchEndTime
      ...
      $http.connPool(sql, arrayParams, (err, result) => {
        if(err) {
          ...
          return $http.writeJson(res, {code: 1, data: resultData, message: '获取订单列表成功'})
        }
      })
    })
  },
