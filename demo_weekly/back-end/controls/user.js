...
const user = {
  ...
  login (req, res) {
    ...
    $http.connPool(sql, arrayParams, (err, result) => {
      if(err) {
        ...
        // if(data.type > 3) {data.role = '客服'}else if(data.type>2){data.role='二级服务商'}else if(data.type>1){data.role='一级服务商'}else{data.role = '管理员'
        if(data.type > 2) {data.role = '客服'}else if(data.type>1){data.role='服务商'}else{data.role = '管理员'
        }
  ...
  /* 用户登出 start */
  // 生成的新的token，并设置一个很短的失效时间，这样当再次访问时，从jwt中解析出的token就会失效，从而达到登出效果。
  logout (req, res) {
    let params = req.body
  ...
  add (req, res) {
    ...
    $http.userVerify(req, res, () => {
      ...
      if(!name || !password) $http.writeJson(res, {code: 2, message:'参数有误'})
        ...
        let arrayParams = [name, password, type, email, parent, tel]
        $http.connPool(sql, arrayParams, (err, result) => {
  ...
  updateInfo (req, res) {
    ...
    $http.userVerify(req, res, () => {
      ...
      if(!id || !name || !password) {$http.writeJson(res, {code: 2, message:'参数有误'})}
        ...
        let arrayParams = [name, password, type, email, parent, tel, id]
        $http.connPool(sql, arrayParams, (err, result) => {
          if(err) return $http.writeJson(res, {code:-2, message:'失败',errMsg: err})
  ...
  list (req, res) {
    ...
    $http.userVerify(req, res, () => {
      ...
      let searchParent = params.searchParent

      let searchZjParent = params.searchZjParent
      let searchJjParent = params.searchJjParent
      ...
      if(!pageNum) {
        ...
        // if(searchParent){
        //   sqlSelectTotal += " and parent like '%"+searchParent+"%'"
        //   sqlSelectList += " and parent like '%"+searchParent+"%'"
        // }

        //所有客服
        if(searchParent){
          sqlSelectTotal += " and parent ="+searchParent
          sqlSelectList += " and parent ="+searchParent
        }
        ...
        // //所有客服数
        // if(searchParent){
        //   sqlSelectTotal += " and parent REGEXP '^"+searchParent+"[0-9.]{2,4}$'"+" and name like 'kf%'"
        //   sqlSelectList += " and parent REGEXP '^"+searchParent+"[0-9.]{2,4}$'"+" and name like 'kf%'"
        // }

        // //直接客服数
        // if(searchZjParent){
        //   sqlSelectTotal += " and parent REGEXP '^"+searchZjParent+"[0-9.]{2}$'"+" and name like 'kf%'"
        //   sqlSelectList += " and parent REGEXP '^"+searchZjParent+"[0-9.]{2}$'"+" and name like 'kf%'"
        // }


        //  //间接客服数
        //  if(searchJjParent){
        //   sqlSelectTotal += " and parent REGEXP '^"+searchJjParent+"[0-9.]{4}$'"+" and name like 'kf%'"
        //   sqlSelectList += " and parent REGEXP '^"+searchJjParent+"[0-9.]{4}$'"+" and name like 'kf%'"
        // }

        let sql= sqlSelectTotal + '; ' + sqlSelectList
        // sql += " order by id desc limit ?,?"; // id倒序排
        sql += " order by id limit ?,?"; // id序排
